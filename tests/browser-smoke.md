...
6. Open **Shots**, switch shots from the bottom filmstrip if more than one exists, and confirm the active shot updates the floating shot card and framing controls. After a pano or frame preview exists, confirm the filmstrip uses that real media rather than a text-only placeholder.

...
6. Open **Shots**, switch shots from the bottom filmstrip if more than one exists, and confirm the active shot updates the docked left shot info sidebar and framing controls. After a pano or frame preview exists, confirm the filmstrip uses that real media rather than a text-only placeholder.
7. Use **Fly Camera** in the bottom action dock (or the shot menu) to reposition the camera, lock it with **Lock View** or a viewport click, and click **Accept Framing**. Verify an advance modal offers Review and reopening fly mode clears the accepted checkpoint.
...
9. In **Review**, export **AI Brief** and confirm the brief-sent checkpoint appears. Import an AI result frame and verify the review card uses the imported media and fills its grid column instead of collapsing to a tiny tile, then confirm the advance modal offers Export.
10. In **Export**, confirm each shot row includes a media thumbnail and the manifest includes `inputs/viewport_clay_motion.mp4`, `inputs/camera_move/clay_start.png`, `inputs/camera_move/cubemap/pz.png`, `metadata/camera_keyframes.json`, `metadata/camera_move_reference_frames.json`, and `metadata/camera_move_cubemap_visibility.json` after a camera move has been exported and linked to a pano. Export with **Export Selected Shots** and confirm the export checkpoint completes for the selected shot.

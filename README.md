...
2. **Reference:** import a styled canonical pano or approve the graybox when iterating without a final AI pano yet. When alignment is needed, yaw and graybox-fade sliders appear on the viewer chrome; full alignment controls stay in the precision drawer. **Projection Assist → Region Fit** in Reference → Precision lets you draw one graybox outline, automatically clone its paired topology onto the styled panorama, and preview a localized correction without pulling unrelated surfaces apart. See [docs/projection-assist.md](docs/projection-assist.md) for the workflow and persistence rules.
...
- `settings`: project and projected-style controls. `settings.projectedStyle.alignments` stores source-panorama-owned local fits, including the target graybox, ordered matches, enabled state, and saved strength.
...
For Projected Style and Projection Assist specifically, verify the panel keeps primary and secondary local-fit cards independent, one valid graybox auto-selects while multiple grayboxes require an explicit choice, a missing target is marked stale, target/source clicks create a match, Preview leaves the project unchanged, strength remains draft-local until Apply, and removing one fit leaves the other source panorama’s fit intact.
...
- Projected Style samples the aligned equirectangular panorama in world space onto existing graybox/imported meshes. It does not create geometry, reconstruct occluded surfaces, or bake UVs; quality is best near each panorama’s own origin and degrades with large translations (stretching / duplicated imagery around occlusions).
- Each reference panorama freezes its projector pose (`pano.origin` / `pano.rotation`) at import/capture time. The scene capture origin (`scene.panoOrigin` / `scene.panoRotation`) is only the marker for new graybox captures and additional origins — moving it after a styled pano is loaded does not rewrite existing projectors (you get a warning first).
- Multi-origin blend modes under Reference → Projected Style combine two panos by inverse-distance weights (depth-free): primary only, secondary only, primary dominant + secondary fill, or secondary dominant + primary fill. True occlusion is not reconstructed.
- Projection quality depends on pano alignment. Use Projection Assist Region Fit under Reference → Precision to draw paired graybox/styled region outlines, preview the localized result on geometry, and apply it only after review. Each fit belongs to its source panorama ID and can target only a valid same-origin graybox render. Projector knobs (opacity, exposure, lighting contribution, fallback, primary/secondary panos, blend mode, and Region Fit strength) live under Reference → Precision as `settings.projectedStyle`.

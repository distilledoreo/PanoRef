...
import { shotStillViewAssetKey } from '../../domain/shotStillViews';
import {
  createLandmark,
  createOriginShot,
  createPanoAsset,
...
import { touchProject } from './touchProject';

/**
 * Project domain slice: persistent document + mutations (objects, panos, shots, landmarks, assets).
 * Owns initial project and actions via Zustand creator closure — not key-picked from a monolith.
...
> = (set, get) => {
  ...
  setPanoOrigin: (origin) => set((state) => {
    ...
    const nextPlan = plan?.trackLiveOrigin
      ...
      : plan;
    // Scene capture origin only ΓÇö never rewrite existing pano projector poses.
    return applyBuildSceneChange(state, {
  ...
  renderGrayboxPano: async () => {
    ...
    try {
      ...
      set((current) => {
        // Only replace grayboxes captured near this origin ΓÇö keep other capture grayboxes.

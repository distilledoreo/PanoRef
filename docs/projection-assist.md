...
4. PanoRef automatically creates paired **Styled** vertices at the same world-space directions as the graybox vertices, converting through both panorama yaws. Raw UVs match only when the panorama orientations match. It then centers the styled viewer on the paired outline and preserves every handle ID, its order, and its paired edge; never redraw the styled outline independently.
